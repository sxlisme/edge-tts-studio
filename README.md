...
声工坊是一款使用 Tauri 2 和 Rust 开发的轻量文字转语音工作台，当前版本为 **1.4.2**，提供 **322 种在线音色**，覆盖 75 种语言。应用直接连接在线语音服务，不需要 Python 环境，也不需要单独部署后端；界面、历史记录和导出的音频文件均由本地桌面应用管理。
...
- 单词配音固定 3 并发并自动重试失败任务，相同内容复用语音素材，可导出 MP3 或 WAV
...
4. 点击“生成循环音频”。应用固定并发生成 3 个语音任务，失败任务自动重试 3 次，并按照“成功数量 / 总数量”显示保留两位小数的进度。
