# 声工坊

声工坊是一款轻量文字转语音工作台，当前版本为 **1.7.2**。桌面端使用 Tauri 2 和 Rust，Android 端使用独立的 Kotlin 原生界面；两端都直接连接在线语音服务，不需要 Python 环境或单独部署后端。

## 界面预览

### 语音工作台

![声工坊主界面](docs/screenshot.png)

### 音色目录与试听

![声工坊音色展示](docs/voice-catalog.png)

### 单词配音

![声工坊单词配音](docs/word-loop.png)

### 音频检测与剪辑

![声工坊音频检测](docs/audio-inspection.png)

## 主要功能

- 322 种在线音色、75 种语言可选，默认使用普通话晓晓音色
- 中文音色名称、头像、语言、性别、声音风格和推荐场景展示
- 自定义语速、音调和音量，输入语言与音色不匹配时主动提醒
- 生成后立即试听，使用真实音频频谱展示播放状态
- MP3 下载、桌面端默认导出文件夹和按时间自动命名
- 最近 50 条本地转换记录，可试听、复用、下载或删除
- Android 原生版提供最多 5000 字的手动输入、最大 1 MB 且最多 5 万字的 TXT 导入、完整在线音色、参数缓存、分段重试、试听和保存到下载目录
- 批量导入 Word、PDF、Markdown 和纯文本文件，生成并发数可在高级设置中配置为 1 至 5
- 批量任务可随时停止；已完成结果保留，已取消任务可重新生成
- 超长文本文件支持最多 50 万字，自动按标点或空格切成不超过 5000 字的分段
- 超长任务按设置的并发数执行，每段开始时即时显示生成状态；单段失败后重试 3 次，成功后按原顺序自动合并为一个 MP3
- 单词配音支持最多 10,000 行内容，可设置音色、循环次数、重复间隔和下一项间隔
- 单词配音按设置的并发数执行并自动重试失败任务，相同内容复用语音素材，可导出 MP3 或 WAV
- 音频检测支持可缩放波形和频谱、超过 3 秒的静音检测、低音量和削波提示
- 支持多音频选区试听、非破坏性剪辑、片段排序拼接并重新导出 WAV
- 音色目录支持试听以及按语言、性别和关键词筛选，语言列表可搜索
- 界面缩放、高对比度、减少动态效果等无障碍设置
- 桌面端提供仅监听本机的 HTTP API
- 桌面大屏和窄屏均可折叠、重新打开侧边菜单，适配不同窗口宽度

## 支持的系统版本

下表描述的是当前仓库实际配置和 GitHub Actions 产出的桌面安装包，不是 Tauri 框架的理论兼容范围。

| 平台 | 最低系统版本 | 处理器架构 | 安装包 | 当前状态 |
| --- | --- | --- | --- | --- |
| macOS | macOS 11 Big Sur | Apple Silicon、Intel | 通用 DMG | 正式支持 |
| Windows | Windows 10 1803 | x64 | NSIS EXE、MSI | 正式支持 |
| Windows | Windows 11 | x64 | NSIS EXE、MSI | 正式支持 |
| Android | Android 8.0 及以上 | 通用 | 原生 APK | 测试版 |

补充说明：

- macOS 安装包使用 Universal Binary，同时包含 `arm64` 和 `x86_64`。
- Windows 当前只构建 x64 制品，暂未提供 Windows ARM64 安装包。
- Windows 7、Windows 8 和 Windows 8.1 不在当前支持范围内。
- Windows 需要 WebView2 Runtime。系统缺少运行环境时，安装程序会尝试联网获取，因此首次安装建议保持网络可用。
- 当前自动构建的 macOS 制品使用临时签名，尚未配置开发者证书和公证流程；正式分发前应补充签名与公证。
- Android APK 使用独立 Kotlin 原生工程构建，目前为调试签名测试版；iOS 尚未提供。

## 下载与安装

### 从 GitHub 下载

1. 打开仓库的 [Releases](https://github.com/sxlisme/edge-tts-studio/releases) 页面。
2. macOS 下载 `.dmg`，Windows 日常安装优先下载 `.exe`。
3. 没有 Release 时，可以在 [Actions](https://github.com/sxlisme/edge-tts-studio/actions) 中打开最近一次成功的 `Desktop packages` 工作流，从 Artifacts 下载对应平台制品。

远程构建的安装包按“平台_产品_版本_其他信息.后缀”命名：

| 平台 | Release 页面显示名称 | 实际下载文件名 |
| --- | --- | --- |
| macOS | `苹果电脑_声工坊_{version}_通用版.dmg` | `macOS_VoiceStudio_{version}_universal.dmg` |
| Windows NSIS | `windows_声工坊_{version}_x64安装版.exe` | `Windows_VoiceStudio_{version}_x64_setup.exe` |
| Windows MSI | `windows_声工坊_{version}_x64中文版.msi` | `Windows_VoiceStudio_{version}_x64_zh-CN.msi` |

GitHub 会清理 Release 真实附件名中的中文字符，因此页面使用中文标签方便识别，实际下载文件使用稳定的 ASCII 名称，避免文件名被替换成下划线。表格中的版本号会随发布版本自动变化。

### macOS

1. 打开下载的 DMG。
2. 将“声工坊”拖入“应用程序”文件夹。
3. 从“应用程序”中启动声工坊。

macOS 11 及以上版本可运行。Apple Silicon 和 Intel Mac 使用同一个通用安装包。

### Windows

1. 普通用户建议运行 NSIS `.exe` 安装包。
2. 需要企业部署或统一安装时可以使用 `.msi`。
3. 安装完成后，从开始菜单启动“声工坊”。

当前 Windows 安装包仅支持 Windows 10 1803 及以上版本和 Windows 11 的 x64 系统。

## 使用方法

### 生成单条语音

1. 在“输入文本”区域输入需要转换的内容，单次最多 10,000 字。
2. 选择语言和音色。中文名称、性别和地区信息会显示在音色选项中。
3. 根据需要调整语速、音调和音量。
4. 点击“生成并试听”。生成期间会展示加载状态和耗时。
5. 生成完成后可以直接试听，或点击“下载 MP3”导出文件。

输入文本和音色语言应尽量一致。例如输入中文时优先选择普通话、粤语或中文（台湾）音色。语言不匹配可能导致发音不自然，部分组合也可能被在线服务拒绝。

### 批量生成文本文件

1. 点击输入区下方的“导入文件”。
2. 选择 `.md`、`.markdown`、`.txt`、`.text`、`.doc`、`.docx` 或 `.pdf` 文件，每次最多导入 50 个文件。
3. 纯文本文件最大 2 MB，Word 和 PDF 文件最大 20 MB。后端会提取 Word/PDF 的可读文本，并自动识别 UTF-8、GBK/GB18030、Big5 和带 BOM 的 UTF-16 等常见纯文本编码。
4. 超过 10,000 字的文件会提示是否跳过，不会静默截断。
5. 点击“开始生成”，应用会按照高级设置中的并发数处理，默认为 1，最高可设为 5。

批量任务中的每条语音成功后会立即保存并显示在历史记录中，不需要等待整个批次结束。运行期间点击“停止生成”会取消当前请求并停止启动后续任务：已经完成的结果继续保留，取消的项目可以再次点击“开始生成”继续处理。

导入文件生成的音频沿用源文件名并改为 `.mp3` 扩展名，例如 `课程讲义.docx` 会生成 `课程讲义.mp3`。目标文件夹已有同名文件时，会依次生成 `课程讲义-1.mp3`、`课程讲义-2.mp3`，不会覆盖原文件。直接粘贴文本生成的音频继续使用原有命名方式。

### 设置默认导出文件夹

桌面端可以在“高级设置”中选择默认导出文件夹。启用后，单条下载和批量生成结果会直接写入该目录；粘贴文本使用生成时间命名，导入文件沿用源文件名，并自动避免覆盖同名文件。

如果文件夹被移动、删除或失去访问权限，应用会停止使用该路径并提示重新选择，不会改写到其他未知位置。

### 生成超长文字语音

1. 点击输入区下方的“50 万字超长生成”，选择文本、Markdown、Word 或 PDF 文件；后端会提取文档文本，或自动识别并转换常见纯文本编码。
2. 文件最多包含 500,000 个字符。确认提示后，应用会按标点或空格自动切分，每段不超过 5,000 字。
3. 应用按照高级设置中的并发数生成分段，并按“成功分段数 / 总分段数”显示进度百分比，保留两位小数。每个请求开始时，任务日志会立即显示“第 X 段生成中…”，并继续记录识别到的编码、总字数、分段数、当前成功数、失败数、重试过程和完成时间。
4. 单个分段生成失败后会自动重试 3 次；仍然失败时终止任务并提示生成失败。
5. 全部分段成功后会按原文顺序合并成一个 MP3，并只保存一条历史记录。设置了默认导出文件夹时，合并文件也会自动导出。

超长文本的生成时间和最终音频体积可能很大。任务期间可以停止生成，已产生但尚未合并的临时分段会自动清理。

Word 或 PDF 文件若包含图片、表格等复杂内容，文本提取可能不完整，转换音频可能异常。建议先手动转换为 TXT 并校验内容，再导入生成。

### Android 应用

Android 端启动后显示文字转语音的核心功能：手动输入最多 5000 字，或导入不超过 1 MB、最多 5 万字的 TXT 文件；通过两个联动下拉框选择语言和音色；在可折叠面板中调整语速、音调和音量；生成后可试听并导出 MP3。TXT 长文本按约 500 字拆成多个请求，优先在 300 至 500 字之间的标点或换行处分段，失败分段最多重试 3 次。

生成期间弹窗显示“生成进度 15%...”及动态点号，并提供停止任务按钮。前 30 秒估算进度从 1% 逐步增长到 20%，之后与真实进度取较大值且不会倒退；真正完成前最高显示 99%，成功后显示 100% 并自动关闭。弹窗会提示长文本生成时间更长且可能转换失败，尚未生成音频时不显示保存按钮。

应用会缓存最后选择的语言、音色、语速、音调和音量，下次启动时自动恢复。启动时会联网读取完整音色目录，普通话、方言、香港粤语和台湾音色统一归入“中文”；获取失败时会弹窗提示当前网络异常，并提供“重试”和“关闭”。导出的音频保存在系统“下载/声工坊”目录。

版本标签会触发 GitHub Actions 构建适用于当前主流 Android 真机的 `arm64-v8a` debug APK，并将可安装文件上传到对应 GitHub Release。该 APK 使用 Android 系统调试签名，适合直接安装测试；发布到应用商店前仍需配置项目长期持有的正式签名密钥。

### 生成单词配音

1. 从左侧菜单进入“单词配音”，在输入框中每行填写一个单词或短句，也可以直接导入 `.txt` 或 `.text` 文件。应用会自动识别 UTF-8、GBK/GB18030、Big5 和带 BOM 的 UTF-16 等常见编码。
2. 最多支持 10,000 行，每行最多 100 字；空行会自动忽略。语言和音色分别选择，并支持在下拉列表中搜索。
3. 根据学习需要调整语速、音调、音量、每项循环次数、重复之间的间隔和下一项开始前的间隔。每项可循环 1 至 20 次，两个间隔均可设置为 0 至 10,000 毫秒。
4. 点击“生成循环音频”。应用按照高级设置中的并发数生成语音，失败任务自动重试 3 次，并按照“成功数量 / 总数量”显示保留两位小数的进度。
5. 全部内容生成成功后，应用按输入顺序和循环设置自动合并，并使用与语音工作台一致的内置播放器试听；通过页面右上角可以选择导出 MP3 或 WAV。

输入中出现相同单词或短句时，应用只生成一次对应的语音素材，并在最终音频中按原行顺序复用，从而减少等待时间。任一内容重试后仍失败时，不会输出不完整的最终音频，失败项会显示在任务列表中，修正后可以再次生成。

### 管理历史记录

- 应用最多保留最近 50 条转换记录。
- 点击历史文本可重新使用原文本和音色。
- 每条记录支持试听、下载和删除。
- 删除记录时，对应的本地音频文件也会一并删除。
- “清空记录”会删除全部历史和对应音频，操作前会再次确认。

### 检测与剪辑音频

1. 从左侧菜单进入“音频检测”，点击“导入音频”，可一次选择多个常见格式的音频文件。
2. 选择素材后，页面会显示可缩放波形与声谱图，并自动检测连续超过 3 秒的静音、整体音量过低和疑似削波。点击异常条目可定位到对应时间。
3. 在波形上拖拽选区，或输入精确的开始与结束秒数；可试听选区后将其加入拼接列表。
4. 拼接列表支持试听、上下调整顺序和删除片段。点击“导出合成 WAV”后，应用按当前顺序输出 44.1 kHz、16-bit PCM WAV。

剪辑采用非破坏性方式，不修改原始文件。同一个拼接列表可以包含多个来源音频的不同选区。

### 浏览音色目录

从左侧菜单进入“音色展示”，可以查看全部音色及其语言、地区、性别、内容类型、声音风格和推荐使用场景。语言筛选支持搜索，并优先展示全部语言、中文、英文、日文和韩文；也可以按关键词和性别继续筛选。

每个音色都提供试听输入框和试听按钮。中文音色默认使用中文测试语，英文等其他音色默认使用英文测试语；测试语可以手动修改，最多 20 字。点击试听后会直接在应用内后台播放，点击“使用”则返回语音工作台并切换到该音色。

### 无障碍与显示设置

“高级设置”提供以下本地持久化选项：

- 整体界面缩放比例
- 高对比度模式
- 减少动态效果
- 语音生成并发数（1 至 5，默认为 1）
- 默认导出文件夹
- 当前操作系统和应用版本信息

设置保存在本机，重新启动应用后仍会生效。

并发数越高，同时发送的在线语音请求越多，可能更快，但也更容易触发请求失败。建议保持默认值 1，仅在网络稳定并确认服务可承受时逐步提高。

## 数据、联网与隐私

声工坊是本地优先应用，但语音合成需要连接在线语音服务。点击生成后，输入文本、音色和语音参数会发送给在线服务以返回音频；除此之外，项目没有接入统计、广告或用户账号服务。

转换历史和音频保存在本机应用数据目录：

- macOS：`~/Library/Application Support/com.sxlisme.voice-studio/`
- Windows：`%APPDATA%\com.sxlisme.voice-studio\`

目录中的 `history.json` 保存记录信息，`audio/` 保存对应 MP3。桌面 HTTP API 只监听 `127.0.0.1`，不会监听局域网或公网地址。

## 桌面 HTTP API

启动桌面应用后，本机 API 地址为 `http://127.0.0.1:8765`。API 与图形界面共享相同的音色、生成逻辑和历史数据。

### 健康检查

```bash
curl http://127.0.0.1:8765/api/health
```

### 获取音色

```bash
curl "http://127.0.0.1:8765/api/voices?locale=zh-CN"
```

### 生成 MP3

```bash
curl -X POST http://127.0.0.1:8765/api/synthesize \
  -H 'Content-Type: application/json' \
  -d '{
    "text": "你好，欢迎使用声工坊。",
    "voice": "zh-CN-XiaoxiaoNeural",
    "rate": "+0%",
    "volume": "+0%",
    "pitch": "+0Hz"
  }' \
  --output speech.mp3
```

生成成功后，响应头 `X-History-Id` 会包含对应的历史记录 ID。

### 历史接口

| 方法 | 地址 | 说明 |
| --- | --- | --- |
| `GET` | `/api/history` | 获取最近 50 条记录 |
| `GET` | `/api/history/{id}/audio` | 获取指定记录的 MP3 |
| `GET` | `/api/history/{id}/audio?download=1` | 以附件形式下载 MP3 |
| `DELETE` | `/api/history/{id}` | 删除记录和对应音频 |
| `DELETE` | `/api/history` | 清空全部记录和音频 |

端口 `8765` 被其他程序占用时，图形界面的 TTS 功能仍可使用，但本地 HTTP API 将无法启动。

## 本地开发

### 环境要求

- Node.js 20+
- Rust stable
- 当前平台对应的 Tauri 2 系统依赖
- macOS 构建需要 Xcode Command Line Tools
- Windows 构建需要 Visual Studio C++ Build Tools 和 WebView2 开发环境

### 启动开发模式

```bash
npm ci
npm run tauri -- dev
```

### 构建当前平台

```bash
npm run tauri -- build
```

### 运行检查

```bash
npm run build
cargo test --manifest-path src-tauri/Cargo.toml
cargo clippy --manifest-path src-tauri/Cargo.toml --all-targets -- -D warnings
```

### 打开开发者工具

桌面 release 包允许打开 Web Inspector，便于排查超长任务和前端异常：

- macOS：按 `Option + Command + I`
- Windows：按 `Ctrl + Shift + I`

超长任务会在 Console 中输出带有 `[超长文字]` 前缀的文件预检、任务启动、分段进度、完成和错误日志。在线语音请求由 Rust 后台发出，因此不会直接出现在 Web Inspector 的 Network 面板中，应结合 Console 日志和页面任务日志判断请求状态。

## 自动构建与发布

仓库包含两个 GitHub Actions 工作流：

- `Desktop packages`：手动触发时生成 macOS 通用 DMG、Windows x64 NSIS EXE 和 MSI，并上传为 Actions Artifacts。
- `Mobile packages`：手动触发 Android APK/AAB 和未签名的 iOS 模拟器构建，用于开发验证。

推送名称符合 `v*` 的 Git 标签时，桌面工作流还会创建 GitHub Release，并把 DMG、EXE 和 MSI 附加到 Release。正式对外发布 macOS 或移动端制品前，需要在仓库中配置对应的开发者证书和签名密钥。

桌面工作流支持通过 Bark 推送构建状态。需要在仓库的 `Settings → Secrets and variables → Actions` 中添加 Repository Secret `BARK_DEVICE_KEY`，内容为 Bark 设备地址最后一段的 Device Key。也可以添加 Repository Variable `BARK_SERVER` 指向自建服务，未配置时默认使用 `https://api.day.app`。工作流会在开始构建以及构建成功或失败时通知，点击通知可直接打开对应的 GitHub Actions 日志。

## 项目结构

```text
.
├── index.html                 # 应用页面结构
├── static/                    # 前端逻辑、样式和本地资源
├── src-tauri/                 # Rust 核心、Tauri 命令和本地 HTTP API
├── docs/                      # README 截图
└── .github/workflows/         # 桌面与移动端自动构建
```

## 常见问题

### 生成失败或接口返回 5xx

先确认设备可以访问在线语音服务，再检查输入内容是否为空、是否超过 10,000 字，以及文本语言与所选音色是否一致。错误提示旁的详情图标可以查看底层返回信息。

### Windows 启动后没有界面

确认系统版本满足 Windows 10 1803 及以上，并安装或修复 WebView2 Runtime，然后重新启动应用。

### 批量任务停止后为什么历史中还有部分记录

停止操作不会删除已经成功的结果。点击停止前已经完成的项目会继续保留在历史记录中，尚未完成或未开始的项目会标记为已取消，可稍后继续生成。

### 默认导出目录失效

目录不存在、不是文件夹或无法写入时，应用会清除失效设置。请在“高级设置”中重新选择一个存在且有写入权限的文件夹。

## 开发者

- 开发者：sxlisme
- 邮箱：blackberrysxl@163.com
