...
声工坊是一款轻量文字转语音工作台，当前版本为 **1.6.1**。桌面端使用 Tauri 2 和 Rust，Android 端使用独立的 Kotlin 原生界面；两端都直接连接在线语音服务，不需要 Python 环境或单独部署后端。
...
- Android 原生版提供最多 5000 字的文本输入、完整在线语言与音色、语速/音调/音量配置、试听和保存到下载目录
...
Android 端启动后只显示文字转语音的核心功能：输入最多 5000 字、通过两个联动下拉框选择语言和音色、调整语速/音调/音量、生成试听以及导出 MP3。应用启动时会联网读取完整音色目录，普通话、方言、香港粤语和台湾音色统一归入“中文”；获取失败时会提示网络异常。导出的音频保存在系统“下载/声工坊”目录。
