...
声工坊是一款轻量文字转语音工作台，当前版本为 **1.7.1**。桌面端使用 Tauri 2 和 Rust，Android 端使用独立的 Kotlin 原生界面；两端都直接连接在线语音服务，不需要 Python 环境或单独部署后端。
...
生成期间弹窗以两位小数显示完成百分比和“已完成片段/全部片段”，并提供停止任务按钮。弹窗会提示长文本生成时间更长且可能转换失败，成功后自动关闭。尚未生成音频时不显示保存按钮。
