...
声工坊是一款轻量文字转语音工作台，当前版本为 **1.7.0**。桌面端使用 Tauri 2 和 Rust，Android 端使用独立的 Kotlin 原生界面；两端都直接连接在线语音服务，不需要 Python 环境或单独部署后端。
...
Android 端启动后显示文字转语音的核心功能：手动输入最多 5000 字，或导入不超过 1 MB、最多 5 万字的 TXT 文件；通过两个联动下拉框选择语言和音色；在可折叠面板中调整语速、音调和音量；生成后可试听并导出 MP3。TXT 长文本会拆成多个请求顺序生成，失败分段最多重试 3 次，界面同步显示生成进度。
